...
glm::vec3 GetGradients(std::shared_ptr<std::vector<FLOAT_TYPE>> terrainView, uint32_t uTerrainWidth, uint32_t uX,
                       uint32_t uY)
{
    ...
    for (int32_t i = 0; i < 4; ++i)
    {
        ...
        auto r1 =
            glm::vec3(uX + offSetXI, uY + offSetYI, lPtr[(uY + offSetYI) * uTerrainWidth + (uX + offSetXI)] * 255);
...
glm::vec3 GetNormal(std::shared_ptr<std::vector<FLOAT_TYPE>> terrainView, uint32_t uTerrainWidth, uint32_t uX,
                    uint32_t uY)
{
    auto lPtr = terrainView->data();
...
glm::vec4 CRenderEngine_Impl::MarchRay_FixedStep(std::shared_ptr<std::vector<FLOAT_TYPE>> terrainView,
                                                 glm::vec3 &v3RayOrigin, glm::vec3 &v3RayDirection,
                                                 uint32_t uTerrainWidth)
{
    ...
    while (true)
    {
        ...
        if (nextSample.x > m_stSettings.uWorldScaleUnits || nextSample.x < 0 ||
            nextSample.z > m_stSettings.uWorldScaleUnits || nextSample.z < 0)
        {
            break;
        }
        // Landscape
...
void CRenderEngine_Impl::MarchedRender(std::string filename, std::shared_ptr<std::vector<FLOAT_TYPE>> terrainView,
                                       uint32_t uTerrainWidth)
{
    // Bad Code
    ...
    // Lookat from origin
    glm::mat4 cameraTransform = glm::lookAtLH(
        v3CameraLocation, glm::vec3(m_stSettings.uWorldScaleUnits / 2, 0, m_stSettings.uWorldScaleUnits / 2),
        glm::vec3(0, 1, 0));
    cameraTransform = glm::inverse(cameraTransform);

    // printf("\n%f %f %f %f\n", cameraTransform[0].r, cameraTransform[0].g, cameraTransform[0].b,
    // cameraTransform[0].w); printf("%f %f %f %f\n", cameraTransform[1].r, cameraTransform[1].g, cameraTransform[1].b,
    // cameraTransform[1].w); printf("%f %f %f %f\n", cameraTransform[2].r, cameraTransform[2].g, cameraTransform[2].b,
    // cameraTransform[2].w); printf("%f %f %f %f\n", cameraTransform[3].r, cameraTransform[3].g, cameraTransform[3].b,
    // cameraTransform[3].w);
    ...
    for (uint32_t a = 0; a < m_stSettings.uHeight / bucketSize; ++a)
    {
        for (uint32_t b = 0; b < m_stSettings.uWidth / bucketSize; ++b)
        {
            ...
            // printf("Bucketing: Xf:%d Yf:%d Xt:%d Yt:%d BId:%d\n",a * bucketSize,b * bucketSize, (a + 1) * bucketSize
            // - 1, (b + 1) * bucketSize - 1, bucketId);
#ifdef NDEBUG
#pragma omp parallel for
            ...
            for (uint32_t p = 0; p < bucketSize * bucketSize; ++p)
            {
                ...
                auto localSpaceGridHit =
                    glm::vec4(localSpaceGridHitX * m_stSettings.fSensorSizeX,
                              -localSpaceGridHitY * m_stSettings.fSensorSizeY, m_stSettings.fCameraDistance, 1.0f);

                // printf("LSG: %f %f %f %f\n", localSpaceGridHit.r, localSpaceGridHit.g, localSpaceGridHit.b,
                // localSpaceGridHit.w);

                auto worldSpaceRayHit = cameraTransform * localSpaceGridHit;

                // printf("WSR: %f %f %f %f\n", worldSpaceRayHit.r, worldSpaceRayHit.g, worldSpaceRayHit.b,
                // worldSpaceRayHit.w);

                auto worldSpaceRayTransform = (worldSpaceRayHit / worldSpaceRayHit.w).xyz() - v3CameraLocation;

                auto worldSpaceRayDirection = glm::normalize(worldSpaceRayTransform);

                // printf("WSD: %d %d\n", pixelX, pixelY);

                // Ray March using quanitised steps
                // image[p] =
                image[pixelY * m_stSettings.uWidth + pixelX] =
                    MarchRay_FixedStep(terrainView, v3CameraLocation, worldSpaceRayDirection, uTerrainWidth).xyz();
            }
        }
    }
    std::ofstream out(filename, std::ios::binary);
    out.write(reinterpret_cast<char *>(&m_stSettings.uWidth), sizeof(uint32_t));

    out.write(reinterpret_cast<char *>(&m_stSettings.uHeight), sizeof(uint32_t));

    out.write(reinterpret_cast<char *>(image.data()), image.size() * sizeof(glm::vec3));
}
