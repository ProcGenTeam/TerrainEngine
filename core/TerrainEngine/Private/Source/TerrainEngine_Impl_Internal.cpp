...
#define OpCase(x, y)                                                                                                   \
    case EOperationTypes::x: {                                                                                         \
        y                                                                                                              \
    }                                                                                                                  \
    break
#define AutoCase(x, ...)                                                                                               \
    case EOperationTypes::x: {                                                                                         \
        Internal_##x(__VA_ARGS__);                                                                                     \
    }                                                                                                                  \
    break
...
void CTerrainEngine_Impl::Internal_LazyEvaluate(FOperation &stOp)
{
    switch (stOp.OpType)
    {
        ...
        AutoCase(AddLayerScalar, stOp.u32Arg1, stOp.u32Arg2, *reinterpret_cast<float *>(&stOp.u32Arg3));
        AutoCase(DivLayerScalar, stOp.u32Arg1, stOp.u32Arg2, *reinterpret_cast<float *>(&stOp.u32Arg3));
        AutoCase(MulLayerScalar, stOp.u32Arg1, stOp.u32Arg2, *reinterpret_cast<float *>(&stOp.u32Arg3));
        AutoCase(SubLayerScalar, stOp.u32Arg1, stOp.u32Arg2, *reinterpret_cast<float *>(&stOp.u32Arg3));
...
CHydraulicErosion *CTerrainEngine_Impl::Internal_GetBestEroder(uint32_t uFilterSize, uint32_t uSeed, int32_t iOffsetX,
                                                               int32_t iOffsetY, FLOAT_TYPE fWaterLevel)
{
// Check GPU
#ifdef TE_USE_GPU
    try
...
// Private Generation
//
void CTerrainEngine_Impl::Internal_Erode(uint32_t uLayerIndex, uint32_t uSteps, uint32_t uTerrainLayerIndex,
                                         uint32_t uFilterSize)
{
    ...
    eroder->Erode(iVecData, iVecData, m_uHeight, m_uWidth, uSteps, tVecData, 0.1f);
...
void CTerrainEngine_Impl::Internal_ErodeByNormals(uint32_t uLayerIndex, uint32_t uSteps)
{
    ...
    for (uint32_t step = 0; step < uSteps; ++step)
    {
        eroder->ErodeByNormals(l_pBuffers[uBufferIndex], l_pBuffers[!uBufferIndex], m_uHeight, m_uWidth,
...
void CTerrainEngine_Impl::Internal_MixLayersCustom(uint32_t uDstLayer, uint32_t uSrcLayer, uint32_t uOtherSrcLayer,
                                                   LayerMixer fnMixingFunction)
{
    auto dstVec = GetInner(this->m_vpvData[uDstLayer]);
    auto srcVec = GetInner(this->m_vpvData[uSrcLayer]);
    auto otsrcVec = GetInner(this->m_vpvData[uOtherSrcLayer]);
