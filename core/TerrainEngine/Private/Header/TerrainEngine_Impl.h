...
private:
    ...
    // We could spherically ray march but that would be expensive as the generation cost of the SDF is high without
    // optimisation
...
protected:
    ...
    virtual CHydraulicErosion *Internal_GetBestEroder(uint32_t uFilterSize, uint32_t uSeed, int32_t iOffsetX,
                                                      int32_t iOffsetY, FLOAT_TYPE fWaterLevel);

    ////// ////// //////
    // Private Generation
    //
    virtual void Internal_Erode(uint32_t uLayerIndex, uint32_t uSteps, uint32_t uTerrainLayerIndex,
    ...
    virtual void Internal_MixLayersCustom(uint32_t uDstLayer, uint32_t uSrcLayer, uint32_t uOtherSrcLayer,
                                          LayerMixer lmMixingFunction);
...
public:
    CTerrainEngine_Impl(FLOAT_TYPE fWaterLevel, uint32_t uWidth, uint32_t uHeight, int32_t iXOffset = 0,
                        int32_t iYOffset = 0, float fScale = 0.001f, uint32_t uOverscan = 2, uint32_t uFilterSize = 2);
    CTerrainEngine_Impl(FLOAT_TYPE fWaterLevel, std::unique_ptr<std::vector<FLOAT_TYPE>> vWorld, uint32_t uWidth,
                        float fScale = 0.001f, uint32_t uOverscan = 2);
    ...
    // Generation
    //
    virtual void Erode(uint32_t uLayerIndex, uint32_t uSteps, uint32_t uTerrainLayerIndex = UINT32_MAX,
