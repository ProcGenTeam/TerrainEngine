...
private:
    ...
    void CreateBuffer(FDeviceBackedBuffer &stBuffer, uint64_t uSize,
                      EGPUBufferTypes eBufferType = EGPUBufferTypes::GPU_ONLY);
    ...
    void UpdateDescriptorSets(uint32_t uDescCount, FDeviceBackedBuffer **ppElements);
...
public:
    CGPUHydraulicErosion(int32_t iOverscan, uint32_t uSeed, int32_t iOffsetX, int32_t iOffsetY,
                         FLOAT_TYPE fWaterLevel = 0.1f);
    ~CGPUHydraulicErosion();

    virtual void Erode(FLOAT_TYPE *pHeight, FLOAT_TYPE *pOut, uint32_t uHeight, uint32_t uWidth, uint32_t uSteps,
                       FLOAT_TYPE *pTerrain, float fScale = 0.1f) override;
    virtual void ErodeByNormals(FLOAT_TYPE *pHeight, FLOAT_TYPE *pOut, uint32_t uHeight, uint32_t uWidth,
                                float fScale = 0.1f) override;
    // virtual void Erode(FLOAT_TYPE *pHeight, FLOAT_TYPE *pOut, uint32_t uHeight, uint32_t uWidth, float fScale =
    // 0.1f);
};
